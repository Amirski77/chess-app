...
export default async function GamesPage() {
  ...
  return (
    <main className="min-h-[calc(100vh-3.5rem)] bg-slate-900 text-slate-100 p-4">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-2xl font-medium mb-4">My Games</h1>

        {error && (
          <p className="text-red-400">Failed to load games: {error.message}</p>
        )}
        ...
        {games && games.length > 0 && (
          <ul className="space-y-2">
            {(games as GameRow[]).map((g) => (
              <li
                key={g.id}
                className="bg-slate-800 rounded p-3 flex justify-between items-center gap-3"
              >
